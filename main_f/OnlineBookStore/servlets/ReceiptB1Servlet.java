...
public class ReceiptB1Servlet extends GenericServlet {
	...
	public void service(ServletRequest req, ServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT * FROM " + ILocationBConstants.TABLE_LOCATIONB + " WHERE "
					+ ILocationBConstants.COLUMN_INTIME + "=? AND " + ILocationBConstants.COLUMN_OUTTIME + "=?");
			...
			if (rs.next()) {
				...
				RequestDispatcher rd = req.getRequestDispatcher("Receipt1b.html");
				rd.include(req, res);
				pw.println("<br/><div class='WERTT'>Total Amount to be paid: Rs." + total + "</div>");
				pw.println("<button>"
						+ "<br/><a href=\"receiptbtwo\">Confirm Booking</a></button>");
