...
public class ReceiptC1Servlet extends GenericServlet {
	...
	public void service(ServletRequest req, ServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT * FROM " + ILocationCConstants.TABLE_LOCATIONC + " WHERE "
					+ ILocationCConstants.COLUMN_INTIME + "=? AND " + ILocationCConstants.COLUMN_OUTTIME + "=?");
			...
			if (rs.next()) {
				...
				RequestDispatcher rd = req.getRequestDispatcher("Receipt1c.html");
				rd.include(req, res);
				pw.println("<br/><div class='WERTT'>Total Amount to be paid: Rs. " + total + "</div>");
				pw.println("<button>"
						+ "<br/><a href=\"receiptctwo\">Confirm Booking</a></button>");
